...
variableNames <- c("p.PC","x","b0","b1","b2","b4","p.proc")
...
xmu[i,yr] <- max(min(x[(i-1),yr] + (b0 + (b1 * x[(i-1),yr]) + (b2 * x[(i-1),yr] ** 2)) + max(0,(b4 * TairMu[i,yr] * D[i,yr])),x[1,yr]),0)
...
foreach(s =1:length(sites)) %dopar% {
  ...
  outputFileName <- paste0(siteName,"_summer_meanTemp_b4_calibration_varBurn.RData")
