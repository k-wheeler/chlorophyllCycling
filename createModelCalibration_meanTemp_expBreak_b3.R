...
variableNames <- c("p.PC","x","b1","b3","p.proc")
...
xmu[i,yr] <- max(min(x[(i-1),yr] + (b1 * x[(i-1),yr]) + max(0,(b3 * Tair[i,yr])),x[1,yr]),0)
...
foreach(s =1:length(sites)) %dopar% {
  ...
  outputFileName <- paste0(siteName,"_meanTempIncludeJuly_expBreak_b3_calibration_varBurn.RData")
