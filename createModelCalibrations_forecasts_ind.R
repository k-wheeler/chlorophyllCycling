...
createChlorophyllCyclingModelCalibration <- function(b0=c(-1,0),b1=c(0,0),b2=c(0,0),b3=c(0,0),b4=c(-1,0),summerOnly=FALSE,dayOnly=FALSE,n=183,s){
  #n.cores <- 24
...
model {
    ...
    if(summerOnly){
      if(dayOnly){
        ...
        initsFileName <- paste0(siteName,"_dayTemp_summer",n,"_expBreak_slope_forecast_",includeB,"_calibration_inits.RData")
      }else{
        outputFileName <- paste0(siteName,"_meanTemp_summer",n,"_expBreak_slope_forecast_",includeB,"_calibration_varBurn.RData")
        initsFileName <- paste0(siteName,"_meanTemp_summer",n,"_expBreak_slope_forecast_",includeB,"_calibration_inits.RData")
      }
      ...
      if(dayOnly){
        ...
        initsFileName <- paste0(siteName,"_dayTemp_fall_expBreak_slope_forecast_",includeB,"_calibration_inits.RData")
        
      }else{
        outputFileName <- paste0(siteName,"_meanTemp_fall_expBreak_slope_forecast_",includeB,"_calibration_varBurn.RData")
        initsFileName <- paste0(siteName,"_meanTemp_fall_expBreak_slope_forecast_",includeB,"_calibration_inits.RData")
      }
    }
    ...
    if(!file.exists(outputFileName)){
      ...
      out.burn <- runForecastIter(j.model=j.model,variableNames=variableNames,
                                  baseNum = 50000,iterSize = 10000,effSize = 5000,
...
ns <- c(119,112,126,183)
ns <- c(84,91,98,205,77)
# n.cores <- 2
# registerDoParallel(cores=n.cores)
#for(n in ns){
#foreach(n=ns) %dopar% {
createChlorophyllCyclingModelCalibration(b3=c(0,1),summerOnly=TRUE,dayOnly=FALSE,n=42,s=10)#19
#}
#createChlorophyllCyclingModelCalibration(b3=c(0,1),summerOnly=TRUE,dayOnly=TRUE)
ns <- c(77,84,91,98,105,112,119,126,183)
